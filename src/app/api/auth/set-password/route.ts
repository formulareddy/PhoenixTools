...
import { setPasswordSchema, validateRequest } from "@/lib/auth-validation"
  ...
  try {
    ...
    if (bearerToken) {
      ...
      if (authUser && authUser.id !== user.id) {
        console.error("[AUTH_SET_PASSWORD] User ID mismatch")
    ...
    if (updateRes.error) {
      console.error("[AUTH_SET_PASSWORD] Failed to set password")
      return NextResponse.json({ error: "Something went wrong. Please try again." }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error("[AUTH_SET_PASSWORD] Unexpected error:", error)
    return NextResponse.json({ error: "Something went wrong. Please try again." }, { status: 500 })
  }
}
